...
#if defined(NDEBUG)
# error "Dhcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd97ae4c4649a548c3aff1e2487f5126fbad134b8e7f11ad9ef52e2089411bce7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dhcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dhcoin Signed Message:\n";
...
{
    // Dhcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Dhcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Dhcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Dhcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dhcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dhcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd97ae4c4649a548c3aff1e2487f5126fbad134b8e7f11ad9ef52e2089411bce7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This Is Dhcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Dhcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DhcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DhcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DhcoinMiner(CWallet *pwallet)
{
    printf("DhcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dhcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DhcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DhcoinMiner, pwallet));
}
